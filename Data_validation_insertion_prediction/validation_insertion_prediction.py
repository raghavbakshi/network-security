...
class DataBaseOperation:
    ...
    def __init__(self):
        ...
        self.badFilePath = "Prediction_rawfiles_validated/Bad_Raw"
        self.goodFilePath = "Prediction_rawfiles_validated/Good_Raw"
        self.logger = logger()
            ...
            for row in session.execute("SELECT keyspace_name, table_name FROM   system_schema.tables;"):
                if row[0] == "prediction":
                    if row[1] == "data":
                        ...
                        session.execute(
                            'CREATE TABLE prediction.data(' + query + ');').one()
                        file = open('Prediction_Logs/DbTableCreateLog.txt', 'a+')
    ...
    def selectingDatafromtableintocsv(self):
        ...
        try:
            ...
            with open(self.fileFromDb + self.fileName, 'w', encoding='utf-8') as csvFile:
                for column_name in session.execute("SELECT * FROM prediction.data;").column_names:
                    csvFile.writelines(column_name + ',')
                csvFile.writelines('\n')
                for i in session.execute("SELECT * FROM prediction.data;").all():
                    for j in i:
