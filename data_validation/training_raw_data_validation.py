...
class Validation:
    ...
    def name_validation(self):
        ...
        try:
            ...
            files = os.listdir(self.training_files)
            self.good_data_folder()
            for file in files:
                if file in lst:
                    shutil.copy("dataset/" + file, "Training_rawfiles_validated/Good_Raw")
                    self.logger.log(f, "Name Validation Complete! File " + "{}".format(file) + "moved to Good Data "
                                                                                               "folder")
                else:
                    shutil.copy("dataset/" + file, "Training_rawfiles_validated/Bad_Raw")
                    self.logger.log(f,
      ...
      Revisions: None
        ...
        try:
            ...
            if not os.path.isdir(path):
                os.mkdir(path)
            path = os.path.join("Training_rawfiles_validated/", "Bad_Raw/")
            if not os.path.isdir(path):
                os.mkdir(path)
        except Exception as e:
    ...
    def delete_good_data_folder(self):
      ...
      Revisions: None
        ...
        try:
            f = open("Training_Logs/GeneralLog.txt", 'a+')
            path = "Training_rawfiles_validated/"
